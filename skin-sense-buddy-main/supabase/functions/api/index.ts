...
async function hashAdminPassword(password: string) {
  const digest = await crypto.subtle.digest("SHA-256", encodeString(password));
...
async function ensureLegacyUserRecord(service: SupabaseClient, user: UserContext) {
  ...
  if (byEmail.data?.id) {
    // Update the existing record to use the correct Supabase Auth UUID
    const updated = await service
      .from("users")
      .update({ id: user.id, password_hash: "supabase-auth-managed" })
      .eq("email", user.email)
      .select("id, email")
      .single();
    if (updated.error) {
      // If update fails (e.g. FK constraints), just return the existing record
      return byEmail.data;
    }
    return updated.data;
  }
...
async function verifyGoogleIdToken(idToken: string) {
  ...
  if (!response.ok) {
    const errMsg = payload.error_description || payload.error || "Google token verification failed";
    console.error(`[Google Auth] tokeninfo failed: ${JSON.stringify(payload)}`);
    return { ok: false as const, error: errMsg };
  }

  const tokenAud = String(payload.aud || "");
  // aud can be a comma-separated list or a single value
  const audList = tokenAud.split(",").map((s: string) => s.trim());
  if (!audList.includes(clientId)) {
    console.error(`[Google Auth] aud mismatch. Token aud: "${tokenAud}", expected: "${clientId}"`);
...
serve(async (req) => {
  ...
  try {
    ...
    if (route === "/auth/signin" && req.method === "POST") {
      ...
      // Supabase Auth failed — check legacy users table (bcrypt password)
      ...
      // Verify bcrypt password using Web Crypto (Deno compatible)
      const bcrypt = await import("https://deno.land/x/bcrypt@v0.4.1/mod.ts");
      const passwordValid = await bcrypt.compare(password, legacyUser.password_hash);
      if (!passwordValid) {
        return json({ error: "Invalid credentials" }, 401);
      }

      // Password is correct — find or create this user in Supabase Auth
      ...
      if (!authUser) {
        // Create in Supabase Auth with a random password (they'll use this flow going forward)
        const tempPw = `Mig-${crypto.randomUUID()}`;
        const created = await service.auth.admin.createUser({ email, password: tempPw, email_confirm: true });
        if (created.error || !created.data.user) {
          return json({ error: "Failed to migrate account. Please contact support." }, 500);
        }
        authUser = created.data.user;
      }

      // Update Supabase Auth password to match what the user just typed
      await service.auth.admin.updateUserById(authUser.id, { password });

      // Now sign in with the correct password
      const migrated = await anon.auth.signInWithPassword({ email, password });
      if (migrated.error || !migrated.data.session) {
        return json({ error: "Sign-in failed after migration. Please try again." }, 401);
      }

      // Sync legacy record
      try {
        await ensureLegacyUserRecord(service, { id: authUser.id, email });
      } catch (_) { /* non-fatal */ }
    ...
    if (route === "/auth/google" && req.method === "POST") {
      ...
      // Find or create user via admin API (no OAuth provider needed)
      ...
      const appUser = { id: authUser.id, email: authUser.email || verified.profile.email };
      try {
        await ensureLegacyUserRecord(service, appUser);
      } catch (error) {
      ...
      // Set full_name from Google profile if not already set
      ...
      // Set a temp password, sign in with it, then immediately replace it
      const tempPassword = `Goog-${crypto.randomUUID()}`;
      await service.auth.admin.updateUserById(authUser.id, { password: tempPassword });
      ...
      // Immediately invalidate the temp password
      await service.auth.admin.updateUserById(authUser.id, { password: `Inv-${crypto.randomUUID()}` });
    ...
    if (route === "/admin/login" && req.method === "POST") {
      ...
      const { data: adminCredential } = await service
      ...
      const passwordMatchesDatabase =
        ...
        adminCredential.password_hash === passwordHash;
