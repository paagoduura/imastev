...
export default function PaymentCallback() {
  ...
  const finalizeAnalysisForScan = async (storedScanId: string, paymentOption: string) => {
    ...
    // Fetch scan details from local API
    const scanResponse = await fetch(`${API_BASE}/scans/${storedScanId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!scanResponse.ok) {
      throw new Error(`Scan not found (${scanResponse.status})`);
    }
  ...
  const finalizePostPayment = async (verifiedTransactionRef: string) => {
    ...
    if (pendingType === 'subscription') {
      ...
      if (storedScanId) setAnalysisScanId(storedScanId);
      sessionStorage.removeItem('pendingPaymentType');
      ...
      try {
        if (storedScanId) {
          ...
          setPostPaymentMessage(
            `Payment verified and your monthly scan plan is now active with ${MONTHLY_SCAN_SUBSCRIPTION_LIMIT} scans.`
          );
        }
      } catch (error) {
              ...
              <CardContent className="p-12 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  <Button
                    onClick={() =>
                      navigate(
                        paymentType === 'telehealth' ? '/telehealth'
                        : paymentType === 'analysis' ? (analysisScanId ? `/results/${analysisScanId}` : '/dashboard')
                        : paymentType === 'subscription' ? (analysisScanId ? `/results/${analysisScanId}` : '/subscription')
                        : paymentType === 'order' ? '/orders'
                        : '/salon-booking'
                      )
                    }
                    className="bg-gradient-to-r from-purple-600 to-amber-500"
                  >
                    {paymentType === 'telehealth' ? 'Go to Telehealth'
                      : paymentType === 'analysis' ? 'View Results'
                      : paymentType === 'subscription' ? (analysisScanId ? 'View Results' : 'View Subscription')
                      : paymentType === 'order' ? 'View Orders'
          ...
          {status === 'pending' && (
            <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm">
              <CardContent className="p-12 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  <Button onClick={verifyPayment} variant="outline">Check Again</Button>
