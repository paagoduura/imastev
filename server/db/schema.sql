...
-- Admin credentials table (for admin login)
...
-- Add tier column to subscription_plans if it doesn't already exist
DO $$ BEGIN
    ALTER TABLE subscription_plans ADD COLUMN IF NOT EXISTS tier VARCHAR(20) DEFAULT 'free';
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

-- Seed default tier values based on plan name if tier is still null/empty
