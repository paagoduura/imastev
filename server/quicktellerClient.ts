...
/**
 * Bug 13 fix: Validate Quickteller credentials at startup.
 * Call this once during server init to surface missing config early,
 * before a real payment request fails with an opaque HTTP error.
 */
export function validateQuicktellerConfig(): void {
  const missing: string[] = [];
  if (!process.env.QUICKTELLER_CLIENT_ID?.trim()) missing.push('QUICKTELLER_CLIENT_ID');
  if (!process.env.QUICKTELLER_CLIENT_SECRET?.trim()) missing.push('QUICKTELLER_CLIENT_SECRET');
  if (!process.env.QUICKTELLER_MERCHANT_CODE?.trim()) missing.push('QUICKTELLER_MERCHANT_CODE');
  if (!process.env.QUICKTELLER_PAYMENT_ITEM_ID?.trim()) missing.push('QUICKTELLER_PAYMENT_ITEM_ID');
  if (missing.length > 0) {
    console.warn(
      `[Quickteller] WARNING: The following required environment variables are not set: ${missing.join(', ')}. ` +
      'Payment endpoints will fail until these are configured.'
    );
  } else {
    console.log(`[Quickteller] Configuration OK (mode: ${getCheckoutMode()})`);
  }
}
